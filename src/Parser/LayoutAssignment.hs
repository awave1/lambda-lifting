module Parser.LayoutAssignment where
 ...
 where
  ...
  res pt st (t0 : ts)
    ...
    = case ts of
      ...
      _ ->
        let
          col     = if null ts then column t0 else column (head ts)
  ...
  res (Just t) (Implicit _n : bs) [] =
    let c = addToken (nextPos t) layoutClose [] in moveAlong bs c []
...
incrGlobal (Pn _ l0 _) i (PT (Pn g l c) t) =
  if l /= l0 then PT (Pn (g + i) l c) t else PT (Pn (g + i) l (c + i)) t
