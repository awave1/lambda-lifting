...
show_fun n (Fun (fname, a1 : args, body)) =
  (spaces n)
    ++ "fun "
    ++ (printer fname)
    ++ "("
    ++ (printer a1)
    ++ (concat (map (\a -> "," ++ (printer a)) args))
    ++ ") = "
    ++ (show_exp' n body)
show_fun n (Fun (fname, [], body)) =
  (spaces n) ++ (printer fname) ++ "() = " ++ (show_exp' n body)
